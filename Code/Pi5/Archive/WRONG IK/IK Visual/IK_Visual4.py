...
class RoboDogSim:
    def __init__(self):
        ...
        self.s_a = Slider(plt.axes([0.09, 0.45, 0.12, 0.03], facecolor=ax_bg), 'Segment a', 2.5, 10, valinit=5.0)
        self.s_b = Slider(plt.axes([0.09, 0.40, 0.12, 0.03], facecolor=ax_bg), 'Segment b', 5, 20, valinit=10.0)
        self.s_c = Slider(plt.axes([0.09, 0.35, 0.12, 0.03], facecolor=ax_bg), 'Segment c', 5, 20, valinit=10.0)
    ...
    def update_plot(self, val):
        ...
        leg_names = ["Front Left", "Back Left", "Back Right", "Front Right"]
        
        print("\n" + "="*80)
        print(f"INPUTS -> X: {target[0]:.2f}, Y: {target[1]:.2f}, Z: {target[2]:.2f} | a: {links[0]:.1f}, b: {links[1]:.1f}, c: {links[2]:.1f}")
        print("-"*80)
        ...
        for i, (leg, s_pos) in enumerate(zip(self.legs, self.shoulders)):
            ...
            # --- IK VARIABLES AND READOUT ---
            # Storing the angles into variables as requested
            final_roll, final_pitch, final_knee = leg.solve_ik(target[0], target[1], target[2], links[0], links[1], links[2])
            
            # Print readout with associated x, y, z, a, b, c
            print(f"{leg_names[i]:12} | Angles -> R: {math.degrees(final_roll):6.1f}°, P: {math.degrees(final_pitch):6.1f}°, K: {math.degrees(final_knee):6.1f}°")
